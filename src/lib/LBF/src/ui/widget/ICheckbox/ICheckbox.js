 ...
 */
LBF.define('ui.widget.ICheckbox.ICheckbox', function (require) {
    ...
    var ICheckbox = Node.inherit({
        ...
        render: function () {
            ...
            if (this.get('selector')) {
                if($selector.is('.lbf-icheckbox')){
                    this.setElement($selector);
                } else {
                    if($selector.parent().is('.lbf-icheckbox')){
                        //无跳动结构
                        this.setElement($selector.parent());
                    }else{
                        //二次渲染
                        $selector.wrap('<div class="lbf-icheckbox"></div>')
                        this.setElement($selector.parent());
                    }
                }
            ...
            //添加定制样式
            this.get('halfchecked') && this.addClass('lbf-icheckbox-halfchecked');
        ...
        isHalfChecked: function () {
            return this.$el.is('.lbf-icheckbox-halfchecked');
        },
        ...
        check: function () {
            ...
            //去除半选态
            this.$el.removeClass('lbf-icheckbox-halfchecked');
        ...
        halfCheck: function(){
            ...
            this.$el.addClass('lbf-icheckbox-halfchecked');
        ...
        pro: function ($, _iCheck, _checkbox, _radio, _checked, _disabled, _type, _click, _touch, _add, _remove, _cursor, _halfchecked) {
            ...
            $.fn[_iCheck] = function (options, fire) {
                ...
                if (/^(check|uncheck|toggle|disable|enable|update|destroy)$/.test(options)) {
                    ...
                    var settings = $.extend({
                            checkedClass: 'lbf-icheckbox-'+ _checked,
                            disabledClass: 'lbf-icheckbox-'+ _disabled,
                            halfcheckedClass: 'lbf-icheckbox-'+ _halfchecked,
                            labelHover: true
                        }, options),

                        selector = settings.handle,
                        hoverClass = settings.hoverClass || 'lbf-icheckbox-hover',
                        focusClass = settings.focusClass || 'lbf-icheckbox-focus',
                        activeClass = settings.activeClass || 'lbf-icheckbox-active',
                        labelHover = !!settings.labelHover,
                        labelHoverClass = settings.labelHoverClass || 'lbf-icheckbox-hover',
                    ...
                    return stack.each(function () {
                        ...
                        helper = $('<ins class="lbf-icheckbox-helper"/>').css(layer).appendTo(parent);
                            ...
                            label.on(_click + '.i mouseenter.i mouseleave.i ' + _touch, function (event) {
                                ...
                                if (!node[_disabled]) {
                                    ...
                                    if (type == _click) {
                                        ...
                                        parent.removeClass('lbf-icheckbox-halfchecked');
            ...
            function on(input, state, keep) {
                ...
                if (node[_disabled] && !!option(input, _cursor, true)) {
                    parent.find('.lbf-icheckbox-helper').css(_cursor, 'default');
                }
                ;
            ...
            function off(input, state, keep) {
                ...
                // Add proper cursor
                if (!node[_disabled] && !!option(input, _cursor, true)) {
                    parent.find('.lbf-icheckbox-helper').css(_cursor, 'pointer');
                }
                ;
    ...
    ICheckbox.include({
        ...
        settings: {
            ...
            template: [
                '<div class="lbf-icheckbox',
                '<% if(checked) { %> ',
                'lbf-icheckbox-checked',
                '<% } %>',
                '<% if(halfchecked) { %> ',
                'lbf-icheckbox-halfchecked',
                '<% } %>',
                '<% if(disabled) { %> ',
                'lbf-icheckbox-disabled',
                '<% } %>',
                '">',
                '<input type="checkbox"',
                '<% if(id) { %> ',
