    ...
    var ComboBox = Node.inherit({
        ...
        renderOptionPanel: function(){
            var combobox = this,
                ...
                defaultIndex = this.get('selectedIndex') || 0,
                defaultValue = (options[defaultIndex] && options[defaultIndex].value) || 0,
        ...
        select: function(index){
            ...
            if(newValue !== oldValue){
                ...
                (!oldValue) ? this.optionPanel.find('a').removeClass('lbf-combobox-item-selected') : this.optionPanel.find('a[data-value='+oldValue+']').removeClass('lbf-combobox-item-selected');
                this.optionPanel.find('a[data-value='+newValue+']').addClass('lbf-combobox-item-selected');
                // todo
                // by amoschen
        ...
        settings: {
            ...
            optionPanelTemplate: [
                '<ul class="lbf-combobox-options">',
                    '<% for(var i=0,len=options.length; i<len; i++){ %>',
                    '<li class="lbf-combobox-option"><a class="lbf-combobox-item<%= defaultValue == options[i].value ? " lbf-combobox-item-selected": "" %>" href="javascript:;" onclick="return false;" title="<%= options[i].text %>" data-value="<%== options[i].value %>"><%= options[i].text %></a></li>',
                    '<% }%>',
                '</ul>'
            ].join(''),
