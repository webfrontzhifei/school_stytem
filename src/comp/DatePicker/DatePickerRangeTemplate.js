 ...
 * @create 6/23/14
 */
LBF.define('qidian.comp.DatePicker.DatePickerRangeTemplate', function(require, exports, module) {
    ...
    return [
        ...
        '<tr>',
        '<% } %>',
        '<td data-date="<%== +cDate %>" class="lbf-date-picker-date lbf-date-picker-date-num <%== classByDate(cDate)%> <%== isDayEnabled(cDate) ? "lbf-date-picker-date-enabled" : "lbf-date-picker-date-disabled"%>"><%== cDate.getDate() %></td>',
        '<% if(day === 6){ %>',
        '</tr>',
        '<% } %>',
        '<% } %>',
